...
void lab_1_1() {
    ...
    stbi_write_png("results/lab_1_1.png", W, H, 3, &image[0], 0);
}
...
void lab_1_2() {
    ...
    stbi_write_png("results/lab_1_2.png", W, H, 3, &image[0], 0);
}
...
void lab_2_1() {
    ...
    for (int i = 0; i < H; i++) {
        for (int j = 0; j < W; j++) {
            ...
            for (int k = 0; k < K; ++k) {
                ...
                color = color + scene.get_color(S, I, ray, 5, 1);
            }
    ...
    stbi_write_png("results/lab_2_1.png", W, H, 3, &image[0], 0);
}
...
void lab_2_2() {
    ...
    Sphere light = Sphere(Vector(-20, -10, 40), 10, Vector(1, 1, 1));
    light.set_light(2 * std::pow(10, 10));
    std::vector<Sphere> spheres = {/*sphere_1,*/ sphere_2, /*sphere_3, sphere_4,*/  light, top, bottom, left, right, front, back};
    Scene scene = Scene(spheres);
    ...
    for (int i = 0; i < H; i++) {
        for (int j = 0; j < W; j++) {
            ...
            for (int k = 0; k < K; ++k) {
                ...
                Ray ray = Ray(Q);
                ray.compute_direction(i + x, j + y, W, H, Q, alpha);
                color = color + scene.get_color(light, ray, 1, 1, false);
            }
            image[(i * W + j) * 3 + 0] = std::min(255., std::pow(color[0]/K, 1./2.2));
            image[(i * W + j) * 3 + 1] = std::min(255., std::pow(color[1]/K, 1./2.2));
            image[(i * W + j) * 3 + 2] = std::min(255., std::pow(color[2]/K, 1./2.2));
    ...
    stbi_write_png("results/lab_2_2.png", W, H, 3, &image[0], 0);
}
